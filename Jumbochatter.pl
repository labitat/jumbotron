...
sub public_hook {
    ...
    if ($target =~ m/#(?:labitat|(?:kn)?test)/i &&
        $msg =~ m/jumbotron/i &&
        $msg =~ m/power|str..?m/i) {
        blipreq_start($server, $target, $msg);
    }
}
...
sub blipreq_start {
  ...
  if ($msg =~ /stram/i) {
    $server->command("MSG $lc_target http://www.stram.cz/");
    return;
  }
  if ($msg =~ /begejstr/i) {
    $server->command("MSG $lc_target http://www.bs.dk/publikationer/andre/faglig/images/s26.jpg");
    return;
  }
  if ($msg =~ /fr[iou]str[iou]m/i) {
    $server->command("MSG $lc_target http://www.jf-koeleteknik.dk/produkter/koele-og-frostanlaeg/");
    return;
  }
  unless ($msg =~ /power/i || $msg =~ /str(?:\x{c3}\x{b8}|\x{f8})m/i) {
    $server->command("MSG $lc_target https://www.youtube.com/watch?v=Ka8bIPqKxiA");
    return;
  }
  ...
  sub {
    ...
    $server->command("MSG $lc_target $text");
  });
